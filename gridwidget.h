...
class GridWidget : public QWidget {
 ...
 public:
  ...
  enum cellPopulationOption { Empty, Random, FromFile }; //Типы заполнения поля

 protected:
  void paintEvent(QPaintEvent* event) override; //Переопределение метода отрисовки
  void mousePressEvent(QMouseEvent* event) override; // Переопределние метода обработки нажатий
 ...
 signals:
  void universeSizeAdjustable(const bool& cmd); // Сигнал о возможности изменения размера поля
 ...
 private:
  ...
  bool doEvolve = false; // Переменная для хранения состояния автоматической эволюции
